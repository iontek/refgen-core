...
class Panel(TenantScopedMixin, Base):
    ...
    code = Column(String(32), unique=True, index=True)   # legacy id e.g. "panel-0024"
    label = Column(String(255), nullable=False)
    type = Column("ptype", String(8), nullable=False, default="DNA")  # DNA/RNA/MIXED
    status = Column(String(16), nullable=False, default="draft")
    details = Column(Text)
    sub_a = Column(String(32))            # set-op provenance (source A)
    sub_b = Column(String(32))            # set-op provenance (source B)
    parent_id = Column(String(32))        # fork lineage (parent panel code)
...
class PanelVersion(TenantScopedMixin, Base):
    ...
    parent_hash = Column(String(64))           # version lineage
